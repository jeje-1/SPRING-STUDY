...
// @ControllerAdvice : com.mc.mvc 패키지 아래에 존재하는 Controller의 공통 작업(예외처리, 데이터바인딩)을 모듈화한 객체
@ControllerAdvice(basePackages = "com.mc.mvc")
public class ExceptionAdvice {
	
	Logger logger = LoggerFactory.getLogger(this.getClass());
	
	// 지정한 에외가 Controller에서 발생하면 어노테이션이 선언된 메서드가 호출된다.
	@ResponseStatus(code=HttpStatus.INTERNAL_SERVER_ERROR)
	@ExceptionHandler(HandlerableException.class)
	public String handlingHandlerableException(HandlerableException e, Model model) {
	...
	//DataAccessExcpeiton : 스프링은 SQLException이 발생할 경우 해당 예외를 rumtimeEscpeiton인
	//DataACcessExcpeiotn으로 wrapping 하여 서비스레이어에 전달
	@ResponseStatus(code=HttpStatus.INTERNAL_SERVER_ERROR)
