...
//						컨트롤러의 메서드의 매개변수가 Map인 경우, @RequestParam을 명시적으로 적용 해줘야한다.
...
//
//		@RequestBody  : request body를 읽어서 자바의 객체에 bind, application/x-www-form-urlEncoded 방식은 지원이 안됨.
// 		@ResponseBody : 컨트롤러의 메서드가 반환하는 값을 답바디에 넣어준다. 
//		@SessionAttribute : servlet의 HttpSession객체에 저장된 속성에 담긴 값을 컨트롤러의 매개변수에 바인드
//		@PathVariable : url템플릿에 담긴 파라미터값을 컨트롤러의 매개변수에 바인드
...
public class MemberController {
	...
	public String authenticateEmail(
		...
		//발송한 이메일에 포함된 url로 사용자가 재용청할 떄 사용자의 정보를 DB에 저장
		//재요청떄까지 데이터를 저장하기 위해 세션객체에 사용자의 정보를 저장
	...
	// /member/signupImpl/{authToken} :/member/signupImpl/* 로 시작하는 모든 요청에 대해 메서드를 호출
	...
	public String signUpImpl(HttpSession session, 
		...
		if(!authToken.equals(sessionToken)) {
			throw new HandlerableException(ErrorCode.EXPIRATION_SIGNUP_TOKEN);
		}
	...
	@GetMapping("/checkId")
	@ResponseBody
	public Map<String, Boolean> checkId(String userId, SignUpForm form){
		// 요청 파라미터로 넘어온 회원의 존재 여부를 응답
