...
public class D_Redirect extends HttpServlet {
	...
	// redierect : 브라우저의 주소창에 /board/list로 변경되어 있음. 
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//요청 재지정 : 클라이언트의 요청에 대해서 작업을 수행 한 뒤, 우리 서버가 제공하는 다른 요청을 서버내부에서 재지정
		//			 get /board-list  => 게시판 목록을 조회 => /board-list/view 라는 요청으로 내부적으로 요청을 재지정 
		
		// /puppy?type=txt == 요청을 재지정 => /puppy/name
		...
		if(uriArr.length == 3) {
			...
			if(type.equals("name")) {
//				브라우저에게 /sping04_servlet/puppy/name 로 다시 요청할 것을 응답
				// http status code : 302
				// location  		: /sping04_servlet/puppy/name
				response.sendRedirect("/spring04_servlet/cat/name");
			}else {
				//브라우저에게 /sping04_servlet/resources/image/image03.jpg로 다시 요청할 것을 응답
				// http status code : 302
				// location  		: /sping04_servlet/resources/image/image03.jpg
		...
		switch (uriArr[3]) {
			...
			// @Webservle()에 지정한 url 패턴은 puppy로 시작하는 모든 요청을 정상적인 요청으로 처리하기 떄문에,
