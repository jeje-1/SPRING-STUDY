...
<h1>JSTL</h1>
<pre>
 	JSP에서 사용하는 스크립트릿의 복잡함을 해결하기 위해 등장한 
</pre>
<h2>1. c:set 변수 생성</h2>
<pre>
	var : 변수명
...
<pre>
	el태그만 사용하면 html태그가 브라우저에 의해서 렌더링된다.
	만약에 el에 의해서 렌더링되는 데이터가, 사용자로부터 입력받은 값일 경우
	악의적인 사용자가 작성한 js코드가 그대로 클라이언트의 화면에서 실행되는 문제가 있다.
	
	el태그는 이러한 script injection 공격에 대처가 되지 않으므로, 
	사용자가 입력했던 문자열 데이터를 화면에 표현할 떄 사용하는 것은 위험하다.
...
<h3>if</h3>
<c:if test = "${num1 < num2 }">
	<span>num1은 num2보다 작습니다.</span>
</c:if>

<c:if test = "${num1 > num2 }">
	<span>num1은 num2보다 큽니다..</span>
</c:if>

<h3>choose when outherwise</h3>
<c:set var="score" value="89"/>
<c:choose>
<c:when test="${score ge 90 }">
	<span>당신의 학접은 A입니다.</span>
</c:when>
<c:when test="${score ge 80 }">
	<span>당신의 학접은 B입니다.</span>
</c:when>
<c:when test="${score ge 70 }">
	<span>당신의 학접은 C입니다.</span>W
</c:when>
<c:when test="${score ge 60 }">
	<span>당신의 학접은 D입니다.</span>
</c:when>
<c:otherwise>
